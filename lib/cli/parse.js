...
function parse(argv) {
  ...
  var args = argv.slice(2),
      ...
      nodemonOptions = {
        watch: [],
        ignore: [],
        nodeArgs: [] // like debug, debug-break
      };
  ...
  if (script === null) {
    console.log('loading script via find');
...
function nodemonOption(options, arg, eatNext) {
  ...
  // line seperation on purpose to help legibility
    ...
    options.delay = parseInt(eatNext(), 10);
  }
...
function findAppScript() {
  ...
  // now try index.js
  if (fs.existsSync('./index.js')) { // FIXME is ./ the right location?
    return 'index.js';
  }
