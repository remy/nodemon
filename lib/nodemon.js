...
function nodemon(settings) {
  ...
  config.load(settings, function (config) {
    ...
    if (config.options.dump) {
      utils.log._log('--------------');
      utils.log._log(util.inspect(config, { depth: null }));
      utils.log._log('--------------');
      utils.log._log(['OS:', process.platform, process.arch].join(' '));
      utils.log._log('node: ' + process.version);
      utils.log._log('nodemon: ' + version);
      utils.log._log('cwd: ' + process.cwd());
      utils.log._log('command: ' + process.argv.join(' '));
      utils.log._log('--------------');
      if (!config.required) {
        process.exit();
      }
