...
bus.on('config:update', function () {
  ...
  if (config.system.useFind) {
    ...
    changeFunction = function (lastStarted, callback) {
      ...
      // if there's an offset, handle the delta calc differently
      // https://github.com/remy/nodemon/pull/566/
      // not 100% sure why though. containers ¯\_(ツ)_/¯
      var delta = offset() === 0 ? (elapsedTime * -1)/1000|0 : (offset() - elapsedTime) + 1000;
      // +1000ms buffer to deal with gradual VM system clock desync

      var findCondition;
      var useReferenceTimestampFile = delta >= 0;

      // if we're a mac (basically) then don't bother running the find
      // when there's no delta - ie. we're in a tight loop
      if (offset() === 0 && delta === 0) {
        return callback([]);
      }
      ...
      if (useReferenceTimestampFile) {
        ...
        findCondition = utils.isMac ? '-mtime ' + (delta) + 's' : '-mmin ' + (delta/60);
      }
