...
bus.on('config:update', function () {
  ...
  if (config.system.useFind) {
    ...
    changeFunction = function (lastStarted, callback) {
      var cmds = [];
      var time = ((Date.now() - config.lastStarted)/1000|0);

      config.dirs.forEach(function(dir) {
        cmds.push('find -L "' + dir + '" ' + ignoredFileTypesForFind(dir) + ' \\( -type f -and -mtime -' + time + 's -print \\)');
      });
...
// Inspired by http://underscorejs.org/docs/underscore.html
function throttle(func, wait, options) {
  ...
  return function() {
    ...
    if (!previous && options.leading === false) previous = new Date().getTime();
  ...
  if (files.length) {
    ...
    if (matched.result.length) {
      if (config.options.delay > 0) {
        if (throttleRestartBus === undefined) {
          throttleRestartBus = throttle(restartBus, config.options.delay, {trailing:false});
        }
        return throttleRestartBus(matched);
      }
      else {
