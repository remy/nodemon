...
import match, { rulesToMonitor } from './match.js';

const debug = Debug('nodemon:watch');
const debugRoot = Debug('nodemon');
const bus = utils.bus;
...
async function watch() {
  ...
  const dirs = [].slice.call(config.dirs);
  ...
  try {
    const total = await new Promise(function (resolve) {
      const dotFilePattern = /[\/\\]\./;
      const ignored = rulesToMonitor(
        [], // not needed
