      ...
      function watch(err, dir) {
        try {
          if (watched.indexOf(dir) === -1 && ignoredFilter(dir)) {
            var watcher = fs.watch(dir, { persistent: false }, function (event, filename) {
...
function filterAndRestart(files) {
  ...
  if (files.length) {
    ...
    var afterIgnore = files.filter(ignoredFilter);
    var watched = [];

    data.ignored = files.length - afterIgnore.length;

    if (config.options.watch.length) {
      watched = files.filter(function (file) {
        return config.options.watch.re && config.options.watch.re.test(file);
      });
    }

    data.watch = watched.length;

    utils.log.detail('changes after filters (all/ignored/watched): ' + [data.all, data.ignored, data.watch].join('/'));

    // reset the last check so we're only looking at recently modified files
    config.lastStarted = Date.now();

    files = watched.concat(afterIgnore);
