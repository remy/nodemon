  ...
  if (utils.isWindows) {
    ...
    if (hasPS) {
      ...
      psTree(child.pid, function (err, kids) {
        var killFN = function (cb) { cb(); };  // Noop
        var signalKills = [];
        var sigusr1Kills = [];
        kids.forEach(function (p) {
          // Java processes don't like sigusr2.
          // Equivalent of a segfault. Replace with SIGUSR1.
          if (signal === 'SIGUSR2' && p.COMM && p.COMM.indexOf('java') > -1) {
            sigusr1Kills.push(p.PID);
          } else {
            signalKills.push(p.PID);
          }
        });
        if (sigusr1Kills.length) {
          killFN = function (cb) {
            spawn('kill', ['-s', 'SIGUSR1'].concat(
              sigusr1Kills)).on('close', cb);
          };
        }
        killFN(function () {
