...
function startNode(execOptions, appargs, options) {
  var executable = execOptions.exec;

  utils.log.status('starting `' + executable + ' ' + execOptions.execArgs.concat(appargs).join(' ') + '`');
  ...
  if (nodeMajor >= 8) {
    child = spawn(executable, execOptions.execArgs.concat(appargs), {
