  ...
  run.kill = function () {
    if (child !== null) {
      ...
      if (utils.isWindows) {
        ...
        // this setTimeout is here to allow the unpipe to finish it's operation,
        // which is only a problem on linux where restarting the child causes
        // EPIPE and ECONNRESET errors. So we unpipe, give it a moment and then
        // kill the child (which then restarts).
        setTimeout(function () {
          child.kill('SIGUSR2');
        }, 200);
      }
