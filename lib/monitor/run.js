...
function run(options) {
  ...
  if (config.options.stdout) {
    if (config.options.childStarted !== false) {
  ...
  if (config.options.childStarted !== false) {
    var childStartedStr = config.options.childStarted || 'nodemon:start:child';
    child.stdout.on('data', function (data) {
      var indexOf = data.toString('utf8').indexOf(childStartedStr);
      console.log("IndexOf", indexOf);
      if (data.toString('utf8').indexOf(childStartedStr) != -1) {
