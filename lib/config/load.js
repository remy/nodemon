...
function loadFile(options, config, dir, ready) {
  ...
  fs.readFile(filename, 'utf8', function (err, jsonData) {
    if (err) {
      var nodemonjsFilename = path.join(dir, 'nodemon.js');
      return exists(nodemonjsFilename, function (exists) {
        if (!exists) {
          return callback({});
        }
        return require.async(nodemonjsFilename, function (jsData) {
          config.loaded.push(nodemonjsFilename);
