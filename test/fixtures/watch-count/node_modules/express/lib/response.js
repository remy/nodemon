...
res.json = function json(obj) {
  ...
  const body = JSON.stringify(val, replacer, spaces);
...
res.sendFile = function sendFile(path, options, fn) {
  ...
  const file = send(req, pathname, options);
...
res.sendfile = function(path, options, fn){
  ...
  // create file stream
  const file = send(req, path, options);
...
res.header = function header(field, val) {
  if (arguments.length === 2) {
    ...
    if ('content-type' == field.toLowerCase() && !/;\s*charset\s*=/.test(val)) {
      const charset = mime.charsets.lookup(val.split(';')[0]);
