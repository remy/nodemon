...
proto.handle = function(req, res, done) {
  ...
  const search = 1 + req.url.indexOf('?');
  const pathlength = search ? search - 1 : req.url.length;
  const fqdn = req.url[0] !== '/' && 1 + req.url.substr(0, pathlength).indexOf('://');
  const protohost = fqdn ? req.url.substr(0, req.url.indexOf('/', 2 + fqdn)) : '';
  const idx = 0;
  const removed = '';
